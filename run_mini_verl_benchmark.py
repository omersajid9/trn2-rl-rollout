...
"""
Benchmark mini_verl GenEngine over input / output length sweeps.

Mirrors the structure of run_benchmark.py (prefill sweep + decode sweep,
COLD/WARM runs, run.log + out.log + per-run metrics.json), adapted for the
synchronous GenEngine API.

Memory logger
-------------
run_benchmark.py uses two processes: ``neuron-monitor -c memory_config.conf``
piped into ``mem_check.py``.  This file replaces that pair with a single
background thread (start_mem_logger / stop_mem_logger) that:

  * runs the same ``neuron-monitor -c memory_config.conf`` command
  * parses each JSON line inside Python — no separate subprocess
  * filters by PID so only this process's NeuronCore usage is reported
  * writes ``[MEM]`` entries to run.log in the exact format make_plots.py
    expects (keys: host_mem_used_gb, swap_used_gb, cpu_user_pct,
    cpu_system_pct, hbm_gb, neuroncore_util_pct)
  * also writes a ``hbm_breakdown`` field with the per-category split
    (tensors / constants / model_code / model_shared_scratchpad /
    runtime_memory) that mem_check.py discards — visible in run.log for
    post-hoc analysis but ignored by make_plots.py

Point-in-time rich snapshots (GC tensor walk + PyTorch allocator) are printed
to stdout / out.log via log_memory_snapshot(neuron_monitor=False) at key
moments.  neuron_monitor=False avoids spawning a competing neuron-monitor
subprocess while the background logger is already running.

Dynamic-shape note
------------------
GenEngine compiles model.forward with backend="neuron", dynamic=False.

  prefill : [batch_size, prefill_chunk_size]  (fixed shape per chunk)
            [batch_size, input_len]            when prefill_chunk_size=0
  decode  : [batch_size, 1]                   always the same
  attn    : [batch_size, max_cache_len]        fixed for the engine lifetime

With prefill_chunk_size=0 each unique input_len triggers a new compilation
on its COLD run.  With a fixed chunk size, non-multiples are left-padded so
the compiled graph is always reused.
"""
...
DEFAULTS = {
    ...
    "batch_size":         1,
    "max_cache_len":      1024,
    "prefill_chunk_size": 0,   # 0 = chunk == prompt_len; each unique len → new NEFF
...
class RunMetrics:
    batch_size:    int   = 0
    input_len:     int   = 0    # content tokens requested
    prompt_len:    int   = 0    # actual padded width fed to the engine
    output_len:    int   = 0    # max_new_tokens
    run_idx:       int   = 0    # 1 = COLD (may compile), 2 = WARM
    total_time_ms: float = 0.0  # wall time after device sync
    tps:           float = 0.0  # batch_size * output_len / (total_time_ms / 1e3)
    # Decode-only inter-token time: (total_ms - prefill_baseline_ms) / (output_len - 1)
    tpt_ms:        float = 0.0
...
# ─── SWEEP ORDERERS ───────────────────────────────────────────────────────────
#
# Each orderer takes a list of lengths and returns the visit sequence.
# In "alternating" mode lengths may appear more than once; callers track the
# per-length visit count and append "_v{n}" to labels / filenames on revisits
# (first visit n=1 → no suffix keeps sequential output clean).
...
def order_alternating(lengths: list[int], seed: int = 0) -> list[int]:
    """Interleave new lengths with the previous one to alternate small/large.

    For [L0, L1, L2, L3, L4] produces:
        L0, L1, L0, L2, L1, L3, L2, L4, L3

    i.e. each new length is immediately followed by a revisit of the previous,
    so the compiled graph for the prior shape stays warm before the next one.
    The first element is visited twice total (once at the start, once as the
    revisit after L1).
    """
    if len(lengths) <= 1:
        return list(lengths)
...
def build_engine(cfg, log_file: Path) -> tuple[GenEngine, int, int]:
    ...
    log(log_file, f"TOKENIZER DONE eos={eos_token_id} pad={pad_token_id}")
...
def prefill_sweep(
    ...
    The traversal order is controlled by cfg.sweep_mode (sequential / random /
    alternating).  In alternating mode some lengths are visited more than once;
    each revisit appends _v2, _v3, ... to the label and metrics filename so
    ...
    for input_len in sweep_order:
        ...
        save_metrics(run_dir / f"metrics_prefill_il{input_len}{vsuf}.json", batch)
        time.sleep(2)
...
def decode_sweep(
    ...
    The traversal order is controlled by cfg.sweep_mode.  Revisited output
    lengths get _v2, _v3, ... suffixes on both labels and metrics filenames.
    """
    ...
    for output_len in sweep_order:
        ...
        save_metrics(run_dir / f"metrics_decode_il{dil}_ol{output_len}{vsuf}.json", batch)
        time.sleep(2)
...
def mini_verl_run(cfg, log_file: Path, run_dir: Path) -> None:
    ...
    if cfg.test in ("decode", "both"):
        ...
        if cfg.decode_input_len not in warm_prefill_ms:
            ...
            log(log_file, f"PREFILL_BASELINE DONE warm_ms={m_warm.total_time_ms:.1f}")
